...
# FUNCTIONS
def create_networkx_object(dbparams, edge_schema, edge_table):
    """
...
def get_od_routes(dbparams, routes_schema, routes_table, row_limit=None, first_row=None):
    """
...
def find_shortest_route(graph, routes_df, col_id, col_orig, col_dest, col_cost):
    """
    ...
    :param col_cost: String containing name of weight column ('travel_time_sec')
    ...
    for i, row in routes_df.iterrows():
        try:
            ...
            p.append({'b_routeid': routes_df[col_id][i], 'shortest_path': str(path), 'time_drive_sec': int(round(time, 0))})
...
def routes2dbtable(dbparams, routes_schema, routes_table, col_cost, paths_list):
    """
    ...
    :param routes_schema: String containing database schema name
    :param routes_table: String containing database table name
    :param col_cost: Name of new column to store route cost (time_drive_sec, time_walk_sec)
    :param paths_list: List with routeids, paths, and travel times (find_shortest_route())
    :return: None
    """
    # SQL to add path and time columns
    sql_add_col1 = "ALTER TABLE {}.{} ADD COLUMN IF NOT EXISTS shortest_path TEXT;".format(routes_schema, routes_table)
    sql_add_col2 = "ALTER TABLE {}.{} ADD COLUMN IF NOT EXISTS {} INT;".format(routes_schema, routes_table, col_cost)
    ...
    with engine.connect() as conn:
        ...
        routes = Table('{}'.format(routes_table), meta,
                       ...
                       Column('shortest_path', String),
                       Column('time_drive_sec', Integer))
        # update columns
        stmt = (update(routes).
                where(routes.c.routeid == bindparam('b_routeid')).
                values(shortest_path=bindparam('shortest_path'), time_drive_sec=bindparam('time_drive_sec')))
